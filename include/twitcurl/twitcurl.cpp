...
bool twitCurl::postReply(std::string& reply, std::string& reply_to_id)
{
    bool retVal = false;
    if( isCurlInit() && reply.length() )
    {
        /* Prepare new status message */
        std::string replyMsg;
        replyMsg = twitCurlDefaults::TWITCURL_STATUSSTRING;
        replyMsg.append(reply.c_str());

        /* Perform POST */
        std::string post_url = twitterDefaults::TWITCURL_STATUSUPDATE_URL \
                               + twitCurlDefaults::TWITCURL_INREPLYTOID \
                               + reply_to_id;
        retVal = performPost( post_url, replyMsg );
        //retVal = performPost( twitterDefaults::TWITCURL_STATUSUPDATE_URL, replyMsg );
    }
    return retVal;
}
...
bool twitCurl::getTweets(const char* url) {
  bool ret_val;
  if (isCurlInit()) {
    prepareStandardParams();
